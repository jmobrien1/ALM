...
jQuery( document ).ready( function() {
	var selected = 0;
	if( jQuery.cookie( 'interface_tab' ) ) {
		selected = jQuery.cookie( 'interface_tab' );
		jQuery.cookie( 'interface_tab', null );
	}
	
	var tabs = jQuery( '#interface_tabs' ).tabs( { selected: selected } );
	
	jQuery( '#wpbody-content form' ).submit( function() {
		var selected = tabs.tabs( 'option', 'selected' );
		jQuery.cookie( 'interface_tab', selected );
	} );
